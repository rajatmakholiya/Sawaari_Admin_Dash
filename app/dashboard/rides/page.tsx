...
export default function RidesPage() {
  ...
  useEffect(() => {
    // Initialize with dummy data if no rides exist
  ...
  const filteredRides = rides.filter(ride => {
    ...
    const matchesCluster = clusterFilter === 'all' || ride.cluster.toLowerCase() === clusterFilter;
    const matchesPayment = paymentFilter === 'all' || ride.paymentMethod === paymentFilter;
    ...
    return matchesSearch && matchesStatus && matchesCluster && matchesPayment && matchesDate;
  });

  const exportToCSV = () => {
    const headers = [
      'Ride ID', 'Driver', 'Vehicle', 'Passenger', 'Phone', 'Pickup', 'Dropoff',
      'Pickup Time', 'Dropoff Time', 'Distance (km)', 'Fare (₹)', 'Status',
      'Payment Method', 'Rating', 'Cluster'
    ];
  ...
  const averageRating = filteredRides.length > 0 
    ? filteredRides.reduce((sum, ride) => sum + ride.rating, 0) / filteredRides.length 
    : 0;
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center space-y-4 lg:space-y-0">
        <div>
          ...
          <p className="text-sm text-gray-600">View and manage all ride history and analytics</p>
        </div>
      ...
      <div className="space-y-4">
        {filteredRides.map((ride) => (
          <Card key={ride.id} className="border-0 shadow-sm hover:shadow-md transition-shadow">
